        ...
        for model_name in groq_models:
            for attempt in range(retries + 1):
                try:
                    ...
                    print(f"[reviewer] groq error on model {model_name}: {e}")
                    if attempt < retries:
                        time.sleep(1)

    print("[reviewer] All LLM provider attempts failed. Falling back to default review.")
    return """## Summary
Automated review failed to complete due to external LLM API availability.
