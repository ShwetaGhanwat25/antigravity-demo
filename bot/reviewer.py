        ...
        for model_name in groq_models:
            for attempt in range(retries + 1):
                try:
                    ...
                    print(f"[reviewer] groq ({model_name}) error: {e}")
                    if attempt < retries:
                        time.sleep(3)

    return """## Summary
Automated code review fallback due to LLM provider unavailability.
...
### 🟡 WARNING
LLM review unavailable. Please review manual verification logs.

### 🟢 INFO
None

## Suggestions
Verify manual tests pass.

## Verdict
⚠️ Needs Changes
LLM service unavailable during automated review."""
