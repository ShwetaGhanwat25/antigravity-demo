        ...
        for model in GROQ_MODELS:
            for attempt in range(retries + 1):
                try:
                    ...
                    print(f"[reviewer] groq error with model {model} (attempt {attempt + 1}): {e}")
                    if attempt < retries:
                        time.sleep(1)

    return """## Summary
Automated review model unavailable.
...
## Verdict
⚠️ Needs Changes
Unable to contact review model. Defaulting securely to Needs Changes."""
