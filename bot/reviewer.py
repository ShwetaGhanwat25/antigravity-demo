...
USE_GEMINI = os.environ.get("IS_GEMINI", "true").strip().lower() != "false"
_provider = "gemini"

if USE_GEMINI:
    from google import genai
    _gemini_client = genai.Client(api_key=os.environ["GEMINI_API_KEY"])
    print("[reviewer] Using Gemini as LLM provider")
else:
    from groq import Groq
    _groq_client = Groq(api_key=os.environ["GROQ_API_KEY"])
    _provider = "groq"
    print("[reviewer] Using Groq as LLM provider")
...
def post_inline_comment(repo, pr_number, commit_sha, path, line, body):
    ...
    if resp.ok:
        print(f"[reviewer] Inline comment posted on {path}:{line}")
    else:
        print(f"[reviewer] Warning: inline comment failed on {path}:{line} — {resp.status_code} {resp.text[:200]}")
...
def notify_google_chat(pr_title, pr_number, pr_url, repo, author, verdict, summary_line,
    ...
    if resp.ok:
        print(f"[reviewer] ✅ Google Chat notification sent successfully (status {resp.status_code})")
    else:
        print(f"[reviewer] ❌ Google Chat notification FAILED — status {resp.status_code}, response: {resp.text[:300]}")
        ...
        for model_name in groq_models:
            for attempt in range(retries + 1):
                try:
                    ...
                    print(f"[reviewer] groq error with model {model_name} (attempt {attempt + 1}): {e}")
                    if attempt < retries:
                        time.sleep(1)

    print("[reviewer] All LLM API attempts failed — returning fallback review")
    return """## Summary
Automated PR review could not be completed because external LLM service was unavailable.
...
### 🟡 WARNING
Unable to perform automated review at this time.

### 🟢 INFO
None

## Suggestions
Please perform a manual review.

## Verdict
⚠️ Needs Changes
LLM review service was unavailable during workflow execution."""
...
def handle_pr(payload, dry_run=False, skip_chat=False):
    ...
    # Inline comments
    print(f"[reviewer] Parsed {len(inline_targets)} inline comment target(s)")
