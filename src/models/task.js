...
function validateEnums(data) {
    if (data.status !== undefined && !Object.values(TaskStatus).includes(data.status)) {
        const err = new Error(`Invalid status. Must be one of: ${Object.values(TaskStatus).join(', ')}`);
        err.status = 400;
        throw err;
    }
    if (data.priority !== undefined && !Object.values(TaskPriority).includes(data.priority)) {
        const err = new Error(`Invalid priority. Must be one of: ${Object.values(TaskPriority).join(', ')}`);
        err.status = 400;
        throw err;
    }
}
...
function update(id, data) {
    ...
    tasks[index] = { ...tasks[index], ...filtered, updatedAt: new Date().toISOString() };
    return tasks[index];
}
